...
The non-interactive default rules may introduce empty shares that do not belong to any message (in the example above, the top-right share is empty). These are zeroes with namespace ID equal to the either . See the [rationale doc](../rationale/message_block_layout.md) for more info.
